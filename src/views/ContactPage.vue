        ...
        <div class="form-section">
          ...
          <div class="contact-form">
            ...
            <div class="form-group">
              ...
              <textarea v-model="form.message" rows="5" placeholder="Mô tả vấn đề của bạn..." :class="{ error: errors.message }"></textarea>
            ...
            <button class="submit-btn" @click="submitForm" :disabled="submitting">
              ...
              <span v-else class="loading-text"><span class="spinner"></span>Đang gửi...</span>
            </button>
        ...
        <div class="chat-section">
          ...
          <div class="chat-widget">
            ...
            <div v-if="!chatStarted" class="chat-intro">
              ...
              <p class="response-time">Thời gian phản hồi trung bình: <strong>~2 phút</strong></p>
              <div class="chat-start-form">
                <input v-model="chatUser.name" type="text" placeholder="Tên của bạn *" />
                <input v-model="chatUser.email" type="email" placeholder="Email (không bắt buộc)" />
                <div v-if="chatError" class="chat-error">{{ chatError }}</div>
                <button class="start-chat-btn" @click="startChat" :disabled="chatLoading">
                  <svg v-if="!chatLoading" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg>
                  <span class="spinner" v-else style="border-color:rgba(255,255,255,.3);border-top-color:#fff;"></span>
            ...
            <div v-else class="chat-interface">
              <div class="chat-header-bar">
                ...
                <button class="end-chat-btn" @click="endChat" title="Kết thúc chat">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                </button>
              </div>

              <div class="messages-area" ref="messagesArea">
                <div v-for="msg in messages" :key="msg.id" class="message" :class="msg.sender === 'customer' ? 'message-user' : 'message-agent'">
              ...
              <div class="chat-input-bar">
                <input v-model="newMessage" type="text" placeholder="Nhập tin nhắn..." @keyup.enter="sendChatMessage" :disabled="sending" />
                <button class="send-btn" @click="sendChatMessage" :disabled="!newMessage.trim() || sending">
...
export default {
  ...
  methods: {
    ...
    validate() {
      ...
      else if (this.form.message.trim().length < 10) this.errors.message = 'Nội dung quá ngắn (tối thiểu 10 ký tự)'
    ...
    async sendChatMessage() {
      ...
      this.scrollBottom()
      try {
        const { data } = await axios.post(`${API}/chat/${this.contactId}/messages`, {
          message: text,
          sender: 'customer',
    ...
    async pollMessages() {
      ...
      try {
        const { data } = await axios.get(`${API}/chat/${this.contactId}/messages`, {
...
.contact-page { font-family: 'Segoe UI', sans-serif; background: #f5f7fa; min-height: 100vh; color: #1a2332; }

.hero-section { background: linear-gradient(135deg, #1a2b4a 0%, #2d4a7a 100%); padding: 60px 24px 80px; text-align: center; position: relative; overflow: hidden; }
.hero-section::before { content: ''; position: absolute; width: 400px; height: 400px; border-radius: 50%; background: rgba(255,255,255,.04); top: -100px; right: -100px; }
...
.form-group input, .form-group select, .form-group textarea { width: 100%; padding: 10px 14px; border: 1.5px solid #e5e7eb; border-radius: 8px; font-size: 14px; color: #111827; background: #fff; outline: none; transition: border-color .2s; font-family: inherit; }
...
.api-error-banner { display: flex; align-items: center; gap: 8px; background: #fef2f2; border: 1px solid #fecaca; border-radius: 8px; padding: 12px 14px; margin-bottom: 12px; font-size: 13px; color: #dc2626; }
.chat-error { font-size: 13px; color: #dc2626; }

.submit-btn { width: 100%; padding: 13px; background: linear-gradient(135deg,#1d4ed8,#2563eb); color: #fff; border: none; border-radius: 10px; font-size: 15px; font-weight: 600; cursor: pointer; transition: opacity .2s; font-family: inherit; }
...
.success-banner { display: flex; align-items: flex-start; gap: 12px; background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 10px; padding: 16px; margin-top: 16px; animation: slideIn .3s ease; }
...
.start-chat-btn { display: flex; align-items: center; justify-content: center; gap: 8px; padding: 12px; background: #1d4ed8; color: #fff; border: none; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; font-family: inherit; transition: background .2s; }
.start-chat-btn:hover { background: #1e40af; }
...
.message-user .msg-time { text-align: right; color: rgba(255,255,255,.6); }
.typing-bubble { display: flex; align-items: center; gap: 4px; padding: 14px; }
