...
onMounted(async () => {
    const coupon = await couponStore.getCouponById(couponId);
    if (!coupon) {
    ...
    form.min_order_value = coupon.min_order_value != null && coupon.min_order_value != 0
...
async function submitForm() {
    ...
    loading.value = true;
    const ok = await couponStore.updateCoupon(couponId, payload);
    loading.value = false;
    ...
    <AdminLayout>
        <div class="page-content">
            ...
            <nav class="breadcrumb">
                ...
                <span class="breadcrumb-current">Chỉnh sửa coupon</span>
            </nav>
            ...
            <div class="content-header">
                <div class="content-titles">
                    ...
                    <p>Cập nhật thông tin mã giảm giá trong hệ thống</p>
                </div>
            </div>

            <!-- Skeleton loading -->
            <div v-if="fetching" class="form-card">
            ...
            <div v-else class="form-card">
                ...
                <div class="form-actions">
                    <RouterLink to="/couponadmin" class="btn-secondary">Hủy</RouterLink>
...
/* Skeleton */
.skeleton-grid { display: flex; flex-direction: column; gap: 24px; }
.skeleton-row { display: grid; grid-template-columns: 1fr 1fr; gap: 24px 32px; }
.skeleton {
    border-radius: 8px;
...
.skeleton-full { height: 56px; }
.skeleton-half { height: 56px; }
@keyframes shimmer {
...
.coupon-left { display: flex; align-items: center; gap: 14px; color: #2563eb; }
.preview-code {
    font-size: 16px;
    font-weight: 800;
    color: #1e40af;
...
.preview-min { font-size: 12px; color: #6b7280; margin-top: 2px; font-weight: 500; }
.coupon-right .preview-value { font-size: 18px; font-weight: 800; color: #16a34a; white-space: nowrap; }
