...
onMounted(async () => {
    ...
    console.log("coupons data:", couponStore.coupons); // xem cấu trúc object
});

function goToCreate() { router.push('/couponadd'); }
function goToEdit(id) { router.push(`/couponedit/${id}`); }

// --- Filter + Sort + Pagination ở client side ---
...
async function handleDelete(id) {
    console.log("id truyền vào:", id); // kiểm tra ở đây
            ...
            <div class="data-card">
                <div class="filter-bar">
                    <div class="filter-search-box">
                        ...
                        <input type="text" placeholder="Tìm kiếm mã coupon..." class="filter-input" v-model="search"
                            @input="onSearchInput" />
                    </div>
                        ...
                        <thead>
                            <tr>
                                ...
                                <th class="sortable" @click="toggleSort('code')">
                                    MÃ COUPON
                                    <span class="sort-icon" :class="{ active: sortKey === 'code' }">{{ sortIcon("code")
                                        }}</span>
                                </th>
                                <th>LOẠI GIẢM GIÁ</th>
                                <th class="sortable" @click="toggleSort('discount_value')">
                                    GIÁ TRỊ
                                    <span class="sort-icon" :class="{ active: sortKey === 'discount_value' }">{{
                            ...
                            <tr v-for="coupon in pagedCoupons" :key="coupon.id">
                                ...
                                <td>
                                    <span class="type-badge"
                                        :class="coupon.discount_type === 'percent' ? 'type-percent' : 'type-fixed'">
                                ...
                                <td class="text-gray">
                                    {{ coupon.min_order_value ? formatMoney(coupon.min_order_value) + 'đ' : '—' }}
                                </td>
                                <td class="col-actions">
                                    <div class="actions-wrapper">
                                        <button class="btn-action edit" @click="goToEdit(coupon.id)" title="Sửa">
                                            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                            </svg>
                                        </button>
                                        <button class="btn-action delete" @click="handleDelete(coupon.id)" title="Xóa">
                                            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="pagination">
                    <span class="pagination-info">
                        Hiển thị {{ rangeStart }} - {{ rangeEnd }} / {{ total }} coupon
                    </span>
                    <div class="pagination-controls">
                        ...
                        <button class="page-btn nav-btn" :disabled="currentPage === lastPage"
