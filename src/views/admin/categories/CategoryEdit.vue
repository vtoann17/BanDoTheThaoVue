...
onMounted(async () => {
  ...
  console.log("cat data:", JSON.parse(JSON.stringify(cat)));
  if (!cat) {
    notify.toastError("Không tìm thấy danh mục");
  ...
  form.name = cat.name;
  form.slug = cat.slug;
  existingImageUrl.value = imageUrl(cat.image);
...
async function submitForm() {
  ...
  loading.value = true;
  const result = await categoryStore.updateCategory(categoryId, fd);
  loading.value = false;

  if (result) router.push("/categoryadmin");
}
...
function resetForm() {
  ...
  if (cat) {
    form.name = cat.name;
    form.slug = cat.slug;
    existingImageUrl.value = cat.image_url ?? cat.image ?? "";
  }
  errors.name = "";
  errors.slug = "";
  imageFile.value = null;
  ...
  <AdminLayout>
    <div class="page-content">
      ...
      <nav class="breadcrumb">
        ...
        <span class="breadcrumb-current">Chỉnh sửa danh mục</span>
      </nav>

      <div class="content-header">
        <div class="content-titles">
          <h2>Chỉnh sửa danh mục</h2>
          <p>Cập nhật thông tin danh mục sản phẩm cho cửa hàng SportGear</p>
        </div>
      </div>
        ...
        <div class="form-grid">
          ...
          <div class="form-group col-span-2">
            ...
            >
              ...
              <!-- Ảnh cũ từ server -->
              <template v-else>
                <img :src="existingImageUrl" class="image-preview" alt="Ảnh hiện tại" />
                <button class="remove-img-btn" @click.stop="removeImage" title="Xóa ảnh">
                  <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" width="16" height="16">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12" />
                  </svg>
                </button>
                <div class="preview-filename">Ảnh hiện tại · click để thay đổi</div>
              </template>
            </div>
          </div>
        </div>
