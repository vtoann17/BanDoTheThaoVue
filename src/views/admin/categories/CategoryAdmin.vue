...
const search = ref("");
const sortValue = ref("id|desc"); // 👈 thêm
...
function toggleSort(key) {
  ...
  // sync lại sortValue cho select khớp
...
function goPage(p) {
  if (p < 1 || p > categoryStore.lastPage) return;
  currentPage.value = p;
}

const pageNumbers = computed(() => {
  const pages = [];
  for (let i = 1; i <= categoryStore.lastPage; i++) pages.push(i);
  return pages;
});

const rangeStart = computed(() =>
  categoryStore.total === 0 ? 0 : (currentPage.value - 1) * perPage + 1
);
const rangeEnd = computed(() =>
  Math.min(currentPage.value * perPage, categoryStore.total)
);

async function handleDelete(id) {
  const ok = await categoryStore.deleteCategory(id);
  if (ok) fetchData();
}
      ...
      <div class="data-card">
        ...
        <div class="table-responsive">
          <table class="data-table">
            ...
            <tbody>
              ...
              <tr
                v-if="
                  categoryStore.total > 0 &&
                  categoryStore.categories.length === 0
                "
              >
                <td colspan="4" class="empty-row">
                  <p
                    style="text-align: center; color: #9ca3af; padding: 24px 0"
                  >
        ...
        <div class="pagination">
          ...
          <div class="pagination-controls">
            ...
            <button
              class="page-btn nav-btn"
              :disabled="currentPage === categoryStore.lastPage"
              @click="goPage(currentPage + 1)"
            >
              <svg
                fill="none"
