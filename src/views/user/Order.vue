...
const selectTab = async (tab) => {
  activeTab.value = tab;
  const params = { per_page: 2, sort_by: "created_at", sort_dir: "desc" };
...
const confirmCancel = async () => {
  ...
  if (result) await selectTab(activeTab.value);
};
          ...
          <div class="order-card" v-for="order in filteredOrders" :key="order.id">
            ...
            <div class="order-body">
              <div class="product-item" v-for="item in order.items" :key="item.id">
                <img
                  :src="item.variant?.img ? `${baseUrl}/storage/${item.variant.img}` : '/placeholder.png'"
                  :alt="item.variant?.product?.name"
                  class="product-img"
                />
                <div class="product-details">
                  <h4 class="product-name">{{ item.variant?.product?.name }}</h4>
                  <p class="product-variant">Phân loại: {{ item.variant?.sku ?? item.variant?.color ?? "—" }}</p>
              ...
              >
                ...
                <span class="cancel-reason-text">{{ order.cancel_reason }}</span>
              </div>
            </div>
            ...
            <div class="order-footer">
              <div class="order-total">
                <div class="total-breakdown">
                  ...
                  <span class="total-sub">{{ Number(order.shipping_fee ?? 0).toLocaleString("vi-VN") }}đ</span>
                </div>
                <div v-if="order.discount > 0" class="total-breakdown">
                  <span class="total-text">Giảm giá:</span>
                  <span class="total-sub discount-text">-{{ Number(order.discount).toLocaleString("vi-VN") }}đ</span>
                </div>
                <div class="total-divider"></div>
                <div class="total-breakdown">
                  <span class="total-text total-label-final">Tổng thanh toán:</span>
                  <span class="total-amount">{{ Number(order.total_amount ?? 0).toLocaleString("vi-VN") }}đ</span>
                </div>
              </div>

              <div class="order-actions">
                <button class="btn-outline" @click="openDetail(order)">Xem chi tiết</button>
                <button
    ...
    <div v-if="showDetailModal" class="modal-overlay" @click.self="showDetailModal = false">
      <div class="detail-modal">
        <div class="detail-modal-header">
          <div>
            <h3 class="detail-modal-title">Chi tiết đơn hàng #{{ selectedOrder?.id }}</h3>
            <p class="detail-modal-date">
              Đặt ngày {{ new Date(selectedOrder?.created_at).toLocaleDateString("vi-VN") }}
            </p>
          </div>
          <button class="detail-modal-close" @click="showDetailModal = false">✕</button>
        </div>
        ...
        <div class="detail-modal-body" v-if="selectedOrder">
          <div class="detail-section">
            <div class="detail-status-row">
              <div class="detail-status-item">
                ...
                <span :class="['status-badge', getStatusInfo(selectedOrder.order_status).class]">
                  {{ getStatusInfo(selectedOrder.order_status).text }}
                </span>
              </div>
              <div class="detail-status-item">
                <span class="detail-status-label">Phương thức TT</span>
                <span class="detail-status-value">{{ selectedOrder.payment_method?.toUpperCase() }}</span>
              </div>
              <div class="detail-status-item">
                <span class="detail-status-label">Trạng thái TT</span>
                <span class="detail-status-value">{{
                  { pending: "Chờ TT", paid: "Đã TT", failed: "Thất bại", refund_pending: "Đang hoàn", refunded: "Đã hoàn tiền" }[selectedOrder.payment_status] || selectedOrder.payment_status
                }}</span>
              </div>
            </div>
          </div>
          ...
          <div
            v-if="selectedOrder.order_status === 'cancelled' && selectedOrder.cancel_reason"
            class="detail-cancel-reason"
          >
            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" width="16" height="16">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
            </svg>
            <span><strong>Lý do hủy:</strong> {{ selectedOrder.cancel_reason }}</span>
          </div>

          <div class="detail-section">
            <h4 class="detail-section-title">Sản phẩm</h4>
            <div class="detail-product-list">
              <div class="detail-product-item" v-for="item in selectedOrder.items" :key="item.id">
                <img
                  :src="item.variant?.img ? `${baseUrl}/storage/${item.variant.img}` : '/placeholder.png'"
                  :alt="item.variant?.product?.name"
                ...
                <div class="detail-product-info">
                  <p class="detail-product-name">{{ item.variant?.product?.name || "—" }}</p>
                  <p class="detail-product-sku">SKU: {{ item.variant?.sku || "—" }}</p>
                  <p class="detail-product-qty">Số lượng: {{ item.quantity }}</p>
                </div>
    ...
    <div v-if="showCancelModal" class="modal-overlay" @click.self="showCancelModal = false">
      <div class="modal">
        ...
        <div class="modal-actions">
          <button class="btn-outline" @click="showCancelModal = false">Quay lại</button>
        ...
        <div v-if="showReviewModal" class="modal-overlay" @click.self="closeReviewModal">
          <div class="modal-box">
            ...
            <div class="modal-product">
              <img
                :src="reviewingItem?.variant?.img ? `${baseUrl}/storage/${reviewingItem.variant.img}` : '/placeholder.png'"
                class="modal-product-img"
                alt=""
              />
              <div>
                <p class="modal-product-name">{{ reviewingItem?.variant?.product?.name }}</p>
                <p class="modal-product-sku">{{ reviewingItem?.variant?.sku ?? reviewingItem?.variant?.color ?? "" }}</p>
              </div>
            </div>
...
.cancel-reason-label { font-size: 13px; font-weight: 600; color: #dc2626; }
.cancel-reason-text  { font-size: 13px; color: #6b7280; }

.order-footer {
  display: flex; justify-content: space-between; align-items: center;
  padding: 10px 16px; border-top: 1px solid #f3f4f6; background-color: #fafbfc;
}
...
.detail-modal-header {
  ...
  padding: 24px 24px 16px; border-bottom: 1px solid #f3f4f6;
  position: sticky; top: 0; background: #fff; z-index: 1;
}
.detail-modal-title  { font-size: 18px; font-weight: 700; color: #111827; }
.detail-modal-date   { font-size: 13px; color: #9ca3af; margin-top: 4px; }
.detail-modal-close  {
  background: none; border: none; font-size: 18px;
  color: #9ca3af; cursor: pointer; padding: 4px 8px; border-radius: 4px;
}
.detail-modal-close:hover { background: #f3f4f6; color: #111; }
.detail-modal-body { padding: 20px 24px; display: flex; flex-direction: column; gap: 16px; }
.detail-section { background: #f9fafb; border-radius: 10px; padding: 16px; }
.detail-section-title {
  font-size: 13px; font-weight: 700; color: #6b7280;
  text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 14px;
}
.detail-status-row  { display: flex; gap: 32px; flex-wrap: wrap; }
.detail-status-item { display: flex; flex-direction: column; gap: 6px; }
.detail-status-label { font-size: 12px; font-weight: 600; color: #9ca3af; text-transform: uppercase; }
.detail-status-value { font-size: 14px; font-weight: 600; color: #111827; }
.detail-cancel-reason {
  display: flex; align-items: center; gap: 8px;
  padding: 12px 14px; background-color: #fef2f2;
  border: 1px solid #fecaca; border-radius: 8px;
  font-size: 14px; color: #dc2626;
}
.detail-product-list { display: flex; flex-direction: column; gap: 12px; }
.detail-product-item {
  display: flex; align-items: center; gap: 14px;
  padding-bottom: 12px; border-bottom: 1px solid #e5e7eb;
}
.detail-product-item:last-child { border-bottom: none; padding-bottom: 0; }
.detail-product-img  { width: 56px; height: 56px; border-radius: 8px; object-fit: cover; border: 1px solid #e5e7eb; flex-shrink: 0; }
.detail-product-info { flex: 1; display: flex; flex-direction: column; gap: 3px; }
.detail-product-name { font-size: 14px; font-weight: 600; color: #111827; }
.detail-product-sku  { font-size: 12px; color: #9ca3af; }
.detail-product-qty  { font-size: 13px; color: #6b7280; }
.detail-product-price { font-size: 14px; font-weight: 700; color: #1a73e8; white-space: nowrap; }
.detail-summary { display: flex; flex-direction: column; gap: 10px; }
.detail-summary-row { display: flex; justify-content: space-between; font-size: 14px; color: #6b7280; }
.detail-summary-total {
  font-size: 16px; font-weight: 700; color: #111827;
  padding-top: 10px; border-top: 1px solid #e5e7eb;
}
.detail-summary-total span:last-child { color: #1a73e8; }
...
.modal-desc  { font-size: 14px; color: #6b7280; }
...
.reason-item:hover { background-color: #f9fafb; border-color: #d1d5db; }
.reason-item input { accent-color: #1a73e8; width: 16px; height: 16px; }
...
.modal-enter-active .modal-box, .modal-leave-active .modal-box { transition: transform 0.25s ease, opacity 0.2s ease; }
...
.star-btn {
  ...
  cursor: pointer; padding: 0 2px; line-height: 1; transition: color 0.15s, transform 0.1s;
}
.star-btn:hover, .star-btn.active { color: #f59e0b; }
.star-btn:hover  { transform: scale(1.2); }
.star-label { font-size: 13px; font-weight: 600; color: #f59e0b; margin-left: 6px; min-width: 76px; }
.review-textarea {
